...
export const useSetupRoom = (playerId: number) => {
  ...
  useEffect(() => {
    const leaveAllRooms = async () => {
      ...
      try {
        await axios.post(`${__API__}/v1/room/leave/all?userId=${playerId}`);
      } catch (error) {
    ...
    const setupRoomAndWebSocket = async () => {
      ...
      if (roomId) {
        const data = (await axios.get(`${__API__}/v1/room/${roomId}`)).data;
      ...
      if (!roomId) {
        ...
        try {
          const availableRoomsResponse = await axios.get(`${__API__}/v1/room/available-for-join?userId=${playerId}`);
        ...
        if (roomId) {
          ...
          await axios.post(`${__API__}/v1/room/${roomId}/join?userId=${playerId}`);
        } else {
          // если нет - создаем и заходим
          const roomResponse = await axios.post(`${__API__}/v1/room/create?userId=${playerId}`);
